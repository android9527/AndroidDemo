...
public class WaterSceneView extends View {
    ...
    private void init() {
        ...
        Bitmap waterBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.depth_water);
        Bitmap foam = BitmapFactory.decodeResource(getResources(), R.drawable.depth_foam);
        setLayerType(View.LAYER_TYPE_HARDWARE, null);
        water = new Water(waterBitmap, foam, getYCoordByPercent(0.65f), getYCoordByPercent(1f), getXCoordByPercent(1f), 6);
        renderables[0] = water;
        Bitmap aura = BitmapFactory.decodeResource(getResources(), R.drawable.depth_sun_aura);
        renderables[1] = new Renderable(aura, getXCoordByPercent(0.5f), getYCoordByPercent(0.35f));

        Bitmap noiseScratch = BitmapFactory.decodeResource(getResources(), R.drawable.depth_noise_scratch);
        Bitmap noiseReg = BitmapFactory.decodeResource(getResources(), R.drawable.depth_noise);
